...
Credentials are read from the server environment only. The browser requests Grab Maps through local `/api/grab/*` proxy endpoints, so bearer tokens are never embedded in frontend code.
