...
def grab_maps_api_key():
    api_key = os.getenv("GRAB_MAPS_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError("GRAB_MAPS_API_KEY is required for Grab Maps requests.")
    return api_key


def grab_maps_mcp_token():
    token = os.getenv("GRAB_MAPS_MCP_TOKEN", "").strip()
    if not token:
        raise RuntimeError("GRAB_MAPS_MCP_TOKEN is required for Grab Maps MCP requests.")
    return token
...
def map_style():
    style = _request_json(f"{BASE_URL}/api/style.json")
    return _proxied_style(style)


def grab_map_asset(asset_path):
    asset_path = asset_path.lstrip("/")
    if asset_path.startswith("vector/"):
        upstream_path = f"api/maps/tiles/v2/{asset_path}"
    elif asset_path.startswith("fonts/"):
        upstream_path = f"api/maps/tiles/v2/{asset_path}"
    elif asset_path.startswith("styles/"):
        upstream_path = f"api/maps/tiles/v2/{asset_path}"
    else:
        raise RuntimeError("Unsupported Grab Maps asset path.")

    request = Request(
        f"{BASE_URL}/{upstream_path}",
        headers={"Authorization": f"Bearer {grab_maps_api_key()}"},
    )
...
def _proxied_style(style):
    style = dict(style)
    style["glyphs"] = "/api/grab/fonts/{fontstack}/{range}.pbf"
    if "fonts" in style:
        style["fonts"] = "/api/grab/fonts/{fontstack}/{language}.ttf"

    sprite = style.get("sprite")
    if isinstance(sprite, str) and "/styles/" in sprite:
        style["sprite"] = _style_proxy_path(sprite)

    sources = style.get("sources")
    if isinstance(sources, dict):
        style["sources"] = {
            key: _proxied_source(source)
            for key, source in sources.items()
        }
    return style


def _proxied_source(source):
...
def _vector_proxy_path(url):
    marker = "/api/maps/tiles/v2/vector/"
    if marker in url:
        return f"/api/grab/vector/{url.split(marker, 1)[1]}"
    return url


def _style_proxy_path(url):
    marker = "/api/maps/tiles/v2/styles/"
    if marker in url:
        return f"/api/grab/styles/{url.split(marker, 1)[1]}"
    return url
